...
class PPHGNet(nn.Module):
    ...
    def __init__(
        ...
        det=False,
        out_indices=None,
        text_rec=None,  # Added to support text_rec parameter
    ):
        super().__init__()
        self.det = det
...
def PPHGNetV2_B4(pretrained=False, use_ssld=False, det=False, **kwargs):
    ...
    Args:
        pretrained: bool=False or str. If `True` load pretrained parameters, `False` otherwise.
                    If str, means the path of the pretrained model.
        use_ssld: bool=False. Whether using distillation pretrained model when pretrained=True.
        det: bool=False. Whether to use detection configuration.
    Returns:
        model: nn.Layer. Specific `PPHGNetV2_B4` model depends on args.
    """
    stage_config_det = {
        # in_channels, mid_channels, out_channels, blocks, downsample
        "stage1": [192, 192, 384, 1, False, 2],
        "stage2": [384, 256, 768, 2, True, 2],
        "stage3": [768, 320, 1152, 3, True, 2],
        "stage4": [1152, 384, 1536, 4, True, 2],
    }

    stage_config_rec = {
        # in_channels, mid_channels, out_channels, blocks, downsample
        "stage1": [192, 192, 384, 1, True, [2, 1]],
        "stage2": [384, 256, 768, 2, True, [1, 2]],
        "stage3": [768, 320, 1152, 3, True, [2, 1]],
        "stage4": [1152, 384, 1536, 4, True, [2, 1]],
    }

    model = PPHGNet(
        stem_channels=[96, 96, 192],
        stage_config=stage_config_det if det else stage_config_rec,
        layer_num=8,
        det=det,
        **kwargs,
